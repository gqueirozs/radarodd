...
.hbadge-d { color: #4d5f7a; background: #141c2a; border: 1px solid rgba(255,255,255,.07); }
.htitle { font-family: var(--font-display); font-weight: 800; color: #f0f4ff; line-height: 1.15; letter-spacing: -.5px; margin-bottom: 10px; }
.hsub   { color: #4d5f7a; font-size: 13px; line-height: 1.6; max-width: 480px; }

/* Filtros de data */
.hfilters { display: flex; gap: 6px; margin-bottom: 0; flex-wrap: wrap; }
.hfilter  { padding: 7px 16px; border-radius: 10px; font-size: 13px; font-weight: 600; border: 1px solid rgba(255,255,255,.07); cursor: pointer; transition: all .15s; background: #0f1520; color: #8b9ab4; }
...
.htime  { font-size: 11px; color: #4d5f7a; font-weight: 500; }
...
.hsinal-prob  { font-size: 11px; color: #8b9ab4; margin-top: 1px; }
...
.hnobet { font-size: 12px; color: #4d5f7a; }
.hbar-info { margin-top: 28px; padding: 13px 16px; background: #0f1520; border-radius: 12px; border: 1px solid rgba(255,255,255,.07); display: flex; align-items: flex-start; gap: 10px; font-size: 12px; color: #4d5f7a; line-height: 1.6; }
...
function Flag({ nome, size }) {
  ...
  if (!logo) return (
    <div style={{ ...st, background: '#1c2537', border: '2px solid rgba(255,255,255,.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: '#4d5f7a', fontSize: size * .4 }}>
      {nome?.charAt(0) || '?'}
    </div>
  );
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', marginTop: 4 }}>
        ...
        <span style={{ fontSize:10, color:'#4d5f7a' }}>Emp {pe}%</span>
  ...
  return (
    <div className={`hcard${temSinais?' vb':''}`} onClick={onClick}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10, gap:8 }}>
          <div style={{ display:'flex', alignItems:'center', gap: isMob?8:12, flex:1 }}>
            ...
            <div>
              ...
              {stC && !isMob && <div style={{ fontSize:10, color:'#4d5f7a', marginTop:2 }}>{stC.gols_marcados}G · #{stC.ranking_fifa}</div>}
            </div>
          </div>
            ...
            <div style={{ display:'flex', gap: isMob?10:8, flexShrink:0, padding: isMob?'0':'0 4px' }}>
              {[{v:o.resultado.casa,l:'1',c:'#00e5a0'},{v:o.resultado.empate,l:'X',c:'#8b9ab4'},{v:o.resultado.fora,l:'2',c:'#4d9fff'}].filter(x=>x.v).map(({v,l,c})=>(
                <div key={l} style={{ textAlign:'center' }}>
                  ...
                  <div style={{ fontSize:9, color:'#4d5f7a', marginTop:2, textTransform:'uppercase', letterSpacing:'.05em' }}>{l}</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ display:'flex', alignItems:'center', gap: isMob?8:12, flex:1, justifyContent:'flex-end' }}>
            <div style={{ textAlign:'right' }}>
              ...
              {stF && !isMob && <div style={{ fontSize:10, color:'#4d5f7a', marginTop:2 }}>{stF.gols_marcados}G · #{stF.ranking_fifa}</div>}
            </div>
      ...
      <div className="hfooter-card">
        <span style={{ fontSize:12, color:'#4d5f7a' }}>
